...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  if (uDistort > 0.0) {
    ...
    uvMod.x += sin(a + iTime * 0.3) * w;
    uvMod.y += cos(b + iTime * 0.25) * w;
  }
...
`;
    ...
    const updateMouseFromClient = (clientX, clientY) => {
      ...
      const localY = (rect.height - (clientY - rect.top)) * scale;

      mouseTargetRef.current = [
        Math.max(0, Math.min(localX, rect.width * scale)),
        Math.max(0, Math.min(localY, rect.height * scale)),
      ];
